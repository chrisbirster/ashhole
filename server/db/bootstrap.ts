...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS room_assignments (id INTEGER PRIMARY KEY AUTOINCREMENT, event_id INTEGER NOT NULL REFERENCES events(id) ON DELETE CASCADE, room TEXT NOT NULL, occupants TEXT NOT NULL, visibility TEXT NOT NULL DEFAULT 'public')`,
  `CREATE TABLE IF NOT EXISTS site_settings (key TEXT PRIMARY KEY, value TEXT NOT NULL)`,
  `UPDATE room_assignments SET visibility = CASE WHEN event_id IN (SELECT id FROM events WHERE visibility = 'member') THEN 'admin' ELSE 'public' END WHERE visibility = 'member'`,
  `UPDATE awards SET visibility = 'admin' WHERE visibility = 'member'`,
  `UPDATE events SET visibility = 'admin' WHERE visibility = 'member'`,
